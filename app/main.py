...
def subset(
    ...
    payload_json = {
        ...
        "coords": {
            lat_name: loaded[lat_name].astype(float).tolist() if lat_name in loaded.coords else None,
            lon_name: loaded[lon_name].astype(float).tolist() if lon_name in loaded.coords else None,
            time_name: loaded[time_name].astype(str).tolist() if time_name in loaded.coords else None,
            depth_name: loaded[depth_name].astype(float).tolist() if depth_name and depth_name in loaded.coords else None,
        },
        "data": loaded.astype(float).tolist(),
    }
...
@app.post("/mean")
def mean(
    coordinates: List[Coordinate] = Body(..., description="Array of {lon, lat} coordinate pairs"),
    dataset: str = Query("reanalysis", description="Dataset key: reanalysis."),
    variable: str = Query(..., description="Variable to extract: so (salinity) or thetao (temperature)."),
    margin: float = Query(..., gt=0, description="Margin in degrees to create square region around each coordinate point."),
    time: str | None = Query(None, description="ISO timestamp or YYYY-MM-DD; nearest match used."),
    depth: float | None = Query(None, description="Depth value; nearest match used."),
) -> Dict[str, float | int]:
    """
    ...
    for idx, coord in enumerate(coordinates):
        ...
        print(f"[DEBUG] Processing point {idx+1}/{len(coordinates)}: lon={lon}, lat={lat}")

        # Validate coordinate bounds
        if not (-180 <= lon <= 180):
            print(f"[DEBUG] Skipping point {idx+1}: Longitude {lon} out of range [-180, 180]")
        ...
        print(f"[DEBUG] Selecting spatial region: lon {min_lon} to {max_lon}, lat {min_lat} to {max_lat}")
